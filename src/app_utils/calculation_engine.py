...
class CalculationEngine:
    ...
    def calculate_benefit(self):
        ...
        if st.button("Calculate Benefits", type="primary", use_container_width=True):
            try:
                country = ssm.SAVED_COUNTRY_ISO3.get()
                ...
                # Determine steps upfront — one per selected benefit
                siikamaki_layers = [v for v in model_class.SIIKAMAKI if v.value] if hasattr(model_class, 'SIIKAMAKI') else []
                global_ess = [es for es in model_class.ECOSYSTEM_SERVICES if es.value and es.global_layer is not None]
                reg_ess_names = list({es.variable.name for es in model_class.ECOSYSTEM_SERVICES if es.value and es.global_layer is None})
                total = len(reg_ess_names) + len(global_ess) + len(siikamaki_layers)
                step = 0

                progress_bar = st.progress(0)

                # Currency conversion happens silently before the bar starts ticking
                conversion_factor = self.convert_to_usd(1, country)

                # Regression predictions — one step per unique benefit name
                for vt in model_class.VALUE_TYPES:
                    ...
                    for es in ess:
                        if vt.variable.name == 'Exchange_Value':
                            step += 1
                            progress_bar.progress(step / total, text=es.variable.name)
                ...
                # Global vector layer ESs (e.g. mangrove coastal protection)
                if global_ess:
                    conversion_factor_global = CurrencyConverter.convert_usd_year(1, country, 2020, 2024)
                    for es in global_ess:
                        step += 1
                        progress_bar.progress(step / total, text=es.variable.name)
                        val = St_Utils.extract_value_from_gpkg(es.global_layer, lat, lon)
                        converted_value = val * conversion_factor_global
                        ...
                        for vt in model_class.VALUE_TYPES:
                            if vt.variable.name == 'Cons_Surplus':
                                prediction_sets[vt.variable.full_name][es.variable.name] = -999
                            if vt.variable.name == 'Exchange_Value':
                                prediction_sets[vt.variable.full_name][es.variable.name] = converted_value

                # Siikamaki COG layer reads
                if siikamaki_layers:
                    siikamaki_benefits = self._calculate_siikamaki(progress_bar, step, total)
    ...
    def _calculate_siikamaki(self, progress_bar, step, total):
        model_class = ssm.MODEL_CLASS.get()
        if ssm.AOI_GDF.get() is not None:
            ...
            for var_obj in siikamaki_layers:
                ...
                progress_bar.progress(step / total, text=layer.full_name)
                value = St_Utils.extract_global_layer_single(layer, ssm.AOI_GDF.get())
                converted_value = value * conversion_factor
                values_per_ha.append({layer.full_name: converted_value})
                var_obj.cons_surplus = converted_value if var_obj.benefit_type == ValueType.CONS_SURPLUS.full_name else -999
                var_obj.exchange_value = converted_value if var_obj.benefit_type == ValueType.EXCHANGE_VALUE.full_name else -999
                step += 1
    ...
    def calculate_costs(self):
        ...
        try:
            ...
            if hasattr(model_class.COST_MODEL, 'GLOBAL_LAYERS'):
                cost_layer_vars = [var_obj for var_obj in model_class.COST_MODEL.GLOBAL_LAYERS if var_obj.value]
                total = len(cost_layer_vars)
                step = 0
                progress_bar = st.progress(0)
                ...
                loc = ssm.PROJECT_LOCATION.get()
                cost_per_ha = []
                for var_obj in cost_layer_vars:
                    layer = var_obj.variable
                    progress_bar.progress(step / total, text=layer.full_name)
                    gcs_path = f"gs://{layer.bucket}/{layer.gcs_path}"
                    value = Spatial.get_value_from_cog(gcs_path, loc['lon'], loc['lat'], loc['area'], band=layer.band)
                    cost_per_ha.append({layer.full_name: value})
                    step += 1
                ...
                try:
                    ...
                    #TODO this needs to be handled at the root and explained to the user that there is no data for EU and US
                ...
                predicted_values = {}
                for nbs in nbss:
                    progress_bar.progress(step / total, text=nbs.variable.name)
                ...
                progress_bar.empty()
                st.success("Calculation Complete!")
                return [{key: float(value)} for key, value in predicted_values.items()]
